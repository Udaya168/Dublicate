    ...
    return (
        <div className="flex h-full w-full">
            ...
            <div className="flex-1 flex flex-col min-w-0 bg-[#0b0d26]">
                ...
                {leftTab === 'titles' && (
                    <div className="flex-1 overflow-y-auto p-2.5 custom-scrollbar">
                        <div className="text-[7.5px] font-bold uppercase tracking-widest text-slate-500 mb-2">Text Styles</div>
                        <div className="grid grid-cols-2 gap-2">
                            {['Modern', 'Cinematic', 'Neon', 'Retro', 'Comic', 'Elegant'].map((style) => (
                                <button key={style} onClick={() => setActiveTool('text-tool')}
                                    className="flex items-center justify-center h-11 rounded-xl bg-white/[0.03] border border-white/[0.07] hover:bg-white/[0.07] hover:border-white/20 transition-all group">
                                    <span className="text-[8.5px] font-bold text-slate-400 group-hover:text-slate-200 uppercase tracking-wider">{style}</span>
                                </button>
                            ))}
                        </div>
                    </div>
...
export const QuickEditStyleScreen = memo(function QuickEditStyleScreen() {
    ...
    const [libraryAssets, setLibraryAssets] = useState<Array<{ id: string, file: File | null, preview: string, type: 'video' | 'image', duration: number }>>([]);
            ...
            <main className="flex-1 flex flex-col overflow-hidden relative z-10">
                ...
                <div className="flex-1 flex flex-col md:flex-row overflow-y-auto md:overflow-hidden border-b border-white/10">
                    ...
                    <aside className="w-full md:w-[420px] flex-none flex bg-[#0B1020]/40 backdrop-blur-md overflow-hidden relative border-r border-white/10 order-2 md:order-1">
                        ...
                        {(() => {
                            const [leftTab, setLeftTab] = useState<'media' | 'stock' | 'audio' | 'titles' | 'transitions' | 'effects' | 'filters' | 'stickers' | 'tools'>('media');
                            const leftTabs = [
                                { id: 'media',       icon: Film,         label: 'Media'       },
                            ...
                            return (
                                <div className="flex h-full w-full">
                                    ...
                                    <div className="flex-1 flex flex-col min-w-0 bg-[#0b0d26]">
                                        ...
                                        {leftTab === 'stickers' && (
                                            <div className="flex-1 overflow-y-auto p-3 custom-scrollbar flex flex-col gap-3 bg-[#0b0d26]">
                                                <div className="text-[8px] font-bold uppercase tracking-widest text-slate-500 mb-1">Stickers & Emojis</div>
                                                <div className="grid grid-cols-4 gap-2">
                                                    {['👍', '🔥', '✨', '🎬', '❤️', '🎉', '🌟', '👀', '💯', '🚀', '💥', '💡'].map((emoji) => (
                                                        <button key={emoji} className="aspect-square rounded-xl bg-white/[0.03] border border-white/5 hover:bg-white/10 text-xl flex items-center justify-center transition-all">
                                                            {emoji}
                                                        </button>
                                                    ))}
                                                </div>
                                        ...
                                        {leftTab === 'titles' && (
                                            <div className="flex-1 overflow-y-auto p-3 custom-scrollbar">
                                                <div className="text-[8px] font-bold uppercase tracking-widest text-slate-500 mb-2">Text Styles</div>
                                                <div className="grid grid-cols-2 gap-2">
                                                    {['Modern', 'Cinematic', 'Neon', 'Retro', 'Comic', 'Elegant'].map((style) => (
                                                        <button key={style} onClick={() => setActiveTool('text-tool')} className="flex items-center justify-center h-12 rounded-xl bg-white/[0.03] border border-white/[0.07] hover:bg-white/[0.07] hover:border-white/20 transition-all group">
                                                            <span className="text-[9px] font-bold text-slate-400 group-hover:text-slate-200 uppercase tracking-wider">{style}</span>
                                                        </button>
                                                    ))}
                                                </div>
                                            </div>
