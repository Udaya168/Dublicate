...
export const TimelineHub = memo(({
  ...
  const clipColors: Record<string, { bg: string; border: string; text: string; dot: string }> = {
    video:  { bg: "bg-teal-950/70",     border: "border-teal-500/40",    text: "text-teal-200",    dot: "bg-teal-400"    },
  ...
  const clipColorsSelected: Record<string, { bg: string; border: string; ring: string }> = {
    video:  { bg: "bg-teal-900/80",    border: "border-teal-300",      ring: "shadow-[0_0_12px_rgba(45,212,191,0.4)]"  },
  ...
  return (
    ...
    >
                      ...
                      {trackClips.map(clip => {
                        ...
                        return (
                          ...
                          >
                             ...
                             {clip.type === "video" && (
                               <div className="absolute inset-0 flex flex-col bg-[#071a19] rounded-xl overflow-hidden">
                                 {/* Clip Header (Top Strip) */}
                                 <div className="h-[22px] bg-[#1a7a59] flex items-center justify-between px-2 shrink-0 z-20 select-none">
                                   ...
                                   <div className="flex items-center gap-1.5 shrink-0">
                                     ...
                                     <span className="text-[8px] font-bold text-emerald-200 bg-[#0e3f2d] px-1 rounded">⚡ Proxy</span>
                                     <span className="text-[8px] font-mono font-medium text-teal-200 bg-[#0e3f2d] px-1 rounded">
                                       {effDur.toFixed(1)}s
                                     </span>
                                   </div>
                                 </div>
                                 ...
                                 {/* Divider Line */}
                                 <div className="h-px bg-[#1a7a59]/60 shrink-0 z-20" />

                                 {/* Audio Waveform representing audio track of video */}
                                 <div className="h-[12px] bg-[#0c2f24] relative overflow-hidden shrink-0 z-20 flex items-center">
                                   <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-85">
                                     <path
                                       ...
                                       stroke={isClipMuted ? "#64748b" : "#22c55e"}
                                       strokeWidth="1.2"
                                       fill="none"
                                     />
                                   </svg>
      ...
      >
        <div className="w-3.5 h-3.5 bg-red-500 rotate-45 border border-red-300 shadow-[0_0_8px_rgba(239,68,68,0.8)]" />
        <div className="absolute top-3.5 bottom-0 left-1/2 -translate-x-1/2 w-0.5 bg-red-500 shadow-[0_0_4px_rgba(239,68,68,0.5)]" />
      </div>
